...
public class Graph {
	...
	public void dijkstra(String startName) {
		...
		// check to see if next path in queue is visited
		while (pq.size() > 0) {
			Path curr = pq.poll();
			if (curr.dest.scratch == 0) {
				Vertex currVert = curr.dest;
				// mark vertex as visited
				currVert.scratch = 1;
				for (Edge currEdge : currVert.adjacent) {
					// update paths to vertices connect to current vertex
					Vertex dest = currEdge.dest;
					double cost = currEdge.cost + curr.weightedCostOfPath; 
									
					// update info if current cost is lower weighted than current path
					if (cost < dest.weightedCostFromStartVertex) {
						dest.prev = currVert;
						dest.weightedCostFromStartVertex = cost;
						pq.add(new Path(dest, cost));
					}
				}
			}
		}
	...
	public void findAllPaths(boolean weighted) {
		...
		for (String name : vertices.keySet()) {
			...
			curr.clearPathInfo();
			// run dijkstra if looking for weighted paths, look for unweighted shortest otherwise
			if (weighted) {
				dijkstra(name);
			} else {
			...
			for (Vertex other : vertices.values()) {
				// if vertex being compared to current is not equal to current and hasn't been
				// checked yet, increment number of vertices connected and weighted/unweighted
				// path lengths
				if (!other.equals(curr) && other.weightedCostFromStartVertex < INFINITY) {
					...
					longest = longestPath(weighted, other, longest);
				}
			}
    ...
    // Helper method for findAllPaths. Finds the longest path between current longest path and 
	// path to current vertex
	private Path longestPath(boolean weighted, Vertex curr, Path longest) {
		Path currPath = getPath(curr.name);
		// set longest equal to current path if current cost is higher, either weighted or 
		// unweighted
		if (((currPath.weightedCostOfPath > longest.weightedCostOfPath) && weighted)
				|| currPath.getNumEdgesInPath() > longest.getNumEdgesInPath()) {
			longest = currPath;
		}
		return longest;
	}
