...
public class FootballRanker {
	...
	private double printRootMeanSquareError(List<String> humanRanks, TreeSet<AllPathsInfo> paths,
		...
		ArrayList<String> predictedRanks = new ArrayList<>();
		// add predicted rankings to list
		for (AllPathsInfo team : paths) {
			predictedRanks.add(team.getName());
		}
		int rank = 1;
		double result = 0;
		for (String curr : humanRanks) {
			// summation part of equation; subtract predicted from human ranks print out 
			// information and square difference. Print out information
			int predictedRank = predictedRanks.indexOf(curr) + 1;
			int diff = rank - predictedRank;
			result += diff * diff;

			if (showResults) {
				System.out.println(String.format("%1$-" + PADDING + "s", curr) + " - actual rank: "
						+ rank + " predicted rank: " + predictedRank);
			}
			rank++;
		}
		// division part of equation, divide by number of teams and take the square root
		...
		int toRound = 10;
		result = Math.round(result * toRound);
		return result / toRound;
	}
